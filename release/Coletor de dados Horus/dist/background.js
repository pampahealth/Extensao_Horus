...
${suffix}`;
  ...
  // supabase-utils.js
  var SUPABASE_URL = "http://116.202.27.216/api/horus/";
  var SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJyb2xlIjoiYW5vbiIsImlzcyI6InN1cGFiYXNlIiwiaWF0IjoxNjQ4NjU4NDAwLCJleHAiOjE5NjQyMDQ4MDB9.6lkYUrysHAhkm014dj7cx3DgYVAYZzwP2QjiNDwA-sk";
    ...
    try {
      if (ultimaColeta.tipo === "formulario_dispensacao" && ultimaColeta.dados) {
        ...
        if (!codigoReceita) {
          ...
          console.warn(
            "\u26A0\uFE0F Campo nuReceita estava vazio, gerando c\xF3digo autom\xE1tico:",
            codigoReceita
          );
        }
        if (!ultimaColeta.url) {
          return {
            sucesso: false,
            erro: "Campo obrigat\xF3rio ausente: url da coleta"
          };
        }
        const payloadDispensacao = {
          ...
          a001_medico_nome: valorTexto(dados.medico) || valorTexto(dados.noPrescritor),
          a001_nmr_conselho: valorTexto(dados.nuConselho),
          a001_codigo_origem_receita: valorTexto(dados.coSeqOrigemReceita),
          a001_local_origem_receita: typeof dados.coSeqOrigemReceita === "object" ? valorTexto(dados.coSeqOrigemReceita?.text) : null,
          a001_cod_subgrupo_origem_receita: valorTexto(
            dados.coSubgrupoOrigemReceita
          ),
          a001_subgrupo_local_origem_receita: typeof dados.coSubgrupoOrigemReceita === "object" ? valorTexto(dados.coSubgrupoOrigemReceita?.text) : null
        };
        ...
        if (dispensacaoExistente) {
          return {
            sucesso: false,
            erro: `Dispensa\xE7\xE3o j\xE1 existe no banco de dados. ID: ${dispensacaoExistente.a001_id}.`,
        ...
        if (dispensacaoError) {
          ...
          if (erroMsg.includes("unq_paciente_receita") || erroMsg.includes("duplicate key") || erroMsg.includes("unique constraint")) {
            return {
              sucesso: false,
              erro: "Dispensa\xE7\xE3o duplicada: j\xE1 existe a mesma combina\xE7\xE3o de paciente, receita e site.",
              duplicado: true
            };
          }
          return {
            sucesso: false,
            erro: "Erro ao salvar dispensa\xE7\xE3o: " + erroMsg
          };
        }
        ...
        if (Array.isArray(dados.itens) && dados.itens.length > 0) {
          ...
          for (const item of dados.itens) {
            const formulaProduto = valorTexto(item.produto?.dsProduto) || valorTexto(item.dsProduto);
            ...
            const itemIndex = item.index !== void 0 && item.index >= 0 ? Number(item.index) : 0;
            if (!formulaProduto) {
              console.warn("\u26A0\uFE0F Item ignorado: f\xF3rmula do produto obrigat\xF3ria", item);
              itensComErro++;
              continue;
            }
            if (!qtDose) {
              console.warn("\u26A0\uFE0F Item ignorado: qt_dose inv\xE1lida", item);
              itensComErro++;
              continue;
            }
            if (!frequencia) {
              console.warn("\u26A0\uFE0F Item ignorado: frequ\xEAncia inv\xE1lida", item);
              itensComErro++;
              continue;
            }
            if (!duracaoTratamDia) {
              console.warn("\u26A0\uFE0F Item ignorado: dura\xE7\xE3o inv\xE1lida", item);
              itensComErro++;
              continue;
            }
            if (!nmrDiasDispensar) {
              console.warn("\u26A0\uFE0F Item ignorado: dias a dispensar inv\xE1lidos", item);
              itensComErro++;
              continue;
            }
            itensPayload.push({
          ...
          if (itensPayload.length > 0) {
            ...
            const itensParaInserir = itensPayload.filter((item) => {
              if (indicesExistentes.has(item.a002_item_index)) {
                console.warn(
                  `\u26A0\uFE0F Item \xEDndice ${item.a002_item_index} j\xE1 existe, ignorando...`
                );
                itensComErro++;
                return false;
              }
            ...
            if (itensParaInserir.length > 0) {
              ...
              const { error: itensError } = await supabase.from("a002_dispensacao_itens").insert(itensParaInserir);
              if (itensError) {
                console.error("\u274C Erro ao salvar itens:", itensError);
                return {
                  sucesso: true,
                  dados: dispensacaoData,
                  aviso: `Dispensa\xE7\xE3o salva, mas houve erro ao salvar itens: ${JSON.stringify(
                    itensError
                  )}. Itens ignorados: ${itensComErro}`,
                  mensagem: `Dispensa\xE7\xE3o ID: ${dispensacaoData.a001_id} salva com aviso nos itens.`
                };
              }
              itensSalvos = itensParaInserir.length;
            }
          } else {
            console.warn("\u26A0\uFE0F Nenhum item v\xE1lido para salvar.");
          }
        }
      ...
      return {
        sucesso: false,
        erro: "Coleta n\xE3o \xE9 uma dispensa\xE7\xE3o do H\xF3rus (tipo formulario_dispensacao). Abra a tela de dispensa\xE7\xE3o e clique em Coletar dados."
      };
    } catch (error) {
  ...
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    console.log("\u{1F4E6} Coleta autom\xE1tica detectada, enviando para Supabase...");
    (async () => {
      try {
        const { ultimaColeta } = await chrome.storage.local.get("ultimaColeta");
        if (!ultimaColeta) {
          console.error("\u274C Nenhum dado encontrado no storage ap\xF3s coleta autom\xE1tica");
        ...
        console.error("\u274C Erro ao processar coleta autom\xE1tica:", error);
        sendResponse({
          sucesso: false,
