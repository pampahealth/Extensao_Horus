...
// supabase-utils.js
var SUPABASE_URL = "https://asxujexunamamwiriqte.supabase.co";
var SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImFzeHVqZXh1bmFtYW13aXJpcXRlIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Njc4MDQyNDcsImV4cCI6MjA4MzM4MDI0N30.CrT3BS4ZsfmLLA-sOJEOzge2Q3C_DX-lO5lONl0Pcj4";
var supabase = createClient(
  SUPABASE_URL,
  SUPABASE_ANON_KEY
);
function normalizarPreco(preco) {
  if (preco === null || preco === void 0)
    return null;
  ...
  try {
    if (ultimaColeta.tipo === "formulario_dispensacao" && ultimaColeta.dados) {
      ...
      const payload = {
        ...
        parcelamento: typeof ultimaColeta.parcelamento === "number" ? ultimaColeta.parcelamento : null,
        prazoEntrega: ultimaColeta.prazoEntrega,
        disponibilidade: ultimaColeta.disponibilidade,
        coletadoEm: ultimaColeta.coletadoEm
      };
      console.log("\u{1F4E4} Payload enviado (formato antigo):", payload);
      const { data, error } = await supabase.from("produtos").insert([payload]);
      console.log("\u{1F4E5} Resposta Supabase:", { data, error });
      if (error) {
        return {
          sucesso: false,
  ...
  btnColetar.addEventListener("click", async () => {
    ...
    try {
      ...
      if (!tab.url || tab.url.startsWith("chrome://") || tab.url.startsWith("chrome-extension://") || tab.url.startsWith("about:")) {
        ...
        alert("\u274C N\xE3o \xE9 poss\xEDvel coletar dados desta p\xE1gina.");
        return;
      }
        ...
        try {
          const resultados = await chrome.scripting.executeScript({
            ...
            func: async () => {
              ...
              const isSiteHorus = location.hostname.includes("horus.saude.gov.br") || document.getElementById("dispensacaoForm") !== null;
              if (isSiteHorus) {
                ...
                if (dadosHorus) {
                  return {
                    ...
                    dados: dadosHorus,
                    coletadoEm: (/* @__PURE__ */ new Date()).toISOString()
                  };
                }
              }
              ...
              let disponibilidade = "N\xE3o informado";
              if (body.includes("em estoque"))
                disponibilidade = "Em estoque";
              else if (body.includes("esgotado") || body.includes("indispon\xEDvel"))
                disponibilidade = "Indispon\xEDvel";
              return {
